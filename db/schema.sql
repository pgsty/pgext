...
-- Extension Universe
-----------------------------------
-- DROP TABLE IF EXISTS pgext.universe CASCADE;

CREATE TABLE pgext.universe
(
    id              INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,                         -- Stable internal identifier.
    name            TEXT        NOT NULL UNIQUE,                                                   -- Extension name used by the control file and CREATE EXTENSION.
    pkg             TEXT        NOT NULL,                                                          -- Project, source distribution, or shared package unit.
    lead_ext        TEXT        NOT NULL REFERENCES pgext.universe(name) DEFERRABLE INITIALLY DEFERRED, -- Primary extension in the extension family.
    category        TEXT        NOT NULL REFERENCES pgext.category(name),                          -- Primary functional category.

    packaged        BOOLEAN     NOT NULL DEFAULT false,                                            -- Whether a distributable package is available.
    lifecycle       TEXT,                                                                          -- Upstream lifecycle: preview, active, deprecated, archived, or abandoned.
    kind            TEXT        NOT NULL,                                                          -- Extension form, such as standard, preload, puresql, or headless.
    kernel          TEXT,                                                                          -- Required or associated PostgreSQL-compatible kernel.
    vendor          TEXT,                                                                          -- Extension, kernel, or managed-service vendor.
    contrib         BOOLEAN     NOT NULL DEFAULT false,                                            -- Whether the extension is part of PostgreSQL contrib.
    lang            TEXT        NOT NULL,                                                          -- Primary implementation language or data format.
    license         TEXT,                                                                          -- SPDX expression or project-specific license identifier.
    tags            TEXT[],                                                                        -- Tags used for search and presentation.

    version         TEXT,                                                                          -- Catalog or control-file extension version.
    url             TEXT        NOT NULL,                                                          -- Preferred user-facing URL.
    repo_url        TEXT,                                                                          -- Source repository or monorepo subdirectory URL.
    home_url        TEXT,                                                                          -- Project, product, or extension homepage.
    doc_url         TEXT,                                                                          -- Official documentation or specification URL.
    license_url     TEXT,                                                                          -- License file or official license page URL.
    control_url     TEXT,                                                                          -- Control file or generated definition URL.
    author_url      TEXT,                                                                          -- Author, maintainer, or publisher URL.
    cargo_url       TEXT,                                                                          -- Cargo.toml or workspace manifest URL.
    pgxn_url        TEXT,                                                                          -- PGXN distribution URL.

    has_bin         BOOLEAN,                                                                       -- Whether the package includes user-facing executables.
    has_lib         BOOLEAN,                                                                       -- Whether the package includes PostgreSQL loadable libraries.
    need_ddl        BOOLEAN     NOT NULL DEFAULT true,                                             -- Whether CREATE EXTENSION or equivalent DDL is required.
    need_load       BOOLEAN,                                                                       -- Whether LOAD, preloading, or another explicit load step is required.
    trusted         BOOLEAN,                                                                       -- Trusted setting from the control file.
    relocatable     BOOLEAN,                                                                       -- Relocatable setting from the control file.
    libs            TEXT[],                                                                        -- Libraries that must be loaded or preloaded.
    schemas         TEXT[],                                                                        -- Schemas fixed, created, or required by the extension.
    pg_ver          TEXT[],                                                                        -- Supported PostgreSQL major versions in ascending order.
    requires        TEXT[],                                                                        -- Forward extension dependencies.
    required_by     TEXT[],                                                                        -- Reverse dependency cache derived from requires.
    see_also        TEXT[],                                                                        -- Related, alternative, or complementary extensions.

    tarball         TEXT,                                                                          -- Source archive used by Pigsty builds.
    pgrx_ver        TEXT,                                                                          -- Effective pgrx build version.
    rpm_ver         TEXT,                                                                          -- RPM catalog version.
    rpm_repo        TEXT,                                                                          -- RPM package source.
    rpm_pkg         TEXT,                                                                          -- RPM package name template.
    rpm_pg          SMALLINT[],                                                                    -- PostgreSQL major versions with RPM packages.
    rpm_deps        TEXT[],                                                                        -- RPM system package dependencies.
    rpm_build       BOOLEAN     NOT NULL DEFAULT false,                                            -- Whether Pigsty maintains an RPM build recipe.
    deb_ver         TEXT,                                                                          -- DEB catalog version.
    deb_repo        TEXT,                                                                          -- DEB package source.
    deb_pkg         TEXT,                                                                          -- DEB package name template.
    deb_pg          SMALLINT[],                                                                    -- PostgreSQL major versions with DEB packages.
    deb_deps        TEXT[],                                                                        -- DEB system package dependencies.
    deb_build       BOOLEAN     NOT NULL DEFAULT false,                                            -- Whether Pigsty maintains a DEB build recipe.

    stars           INTEGER,                                                                       -- Latest observed upstream star count.
    watchers        INTEGER,                                                                       -- Latest observed watcher or subscriber count.
    forks           INTEGER,                                                                       -- Latest observed fork count.
    repo_created_at DATE,                                                                          -- Upstream repository creation date.
    last_commit     DATE,                                                                          -- Latest known upstream commit date.
    last_release    DATE,                                                                          -- Latest known release, tag, or PGXN publication date.
    last_active     DATE,                                                                          -- Latest credible upstream activity date.
    checked_at      DATE,                                                                          -- Date when upstream metadata was last verified.

    en_desc         TEXT,                                                                          -- Short English description.
    zh_desc         TEXT,                                                                          -- Short Chinese description.
    comment         TEXT        CONSTRAINT universe_comment_single_line CHECK (comment !~ E'[\r\n]'), -- Maintenance notes, caveats, exceptions, and manual decisions.
    extra           JSONB       NOT NULL DEFAULT '{}'::JSONB,                                      -- Compact attributes not promoted to dedicated columns.
    mtime           DATE        NOT NULL DEFAULT CURRENT_DATE                                      -- Date of the latest catalog-level change to the row.
);
...
COMMENT ON COLUMN pgext.universe.category        IS 'Primary functional category.';
COMMENT ON COLUMN pgext.universe.packaged        IS 'Whether a distributable package is available.';
COMMENT ON COLUMN pgext.universe.lifecycle       IS 'Upstream lifecycle: preview, active, deprecated, archived, or abandoned.';
COMMENT ON COLUMN pgext.universe.kind            IS 'Extension form, such as standard, preload, puresql, or headless.';
COMMENT ON COLUMN pgext.universe.kernel          IS 'Required or associated PostgreSQL-compatible kernel.';
COMMENT ON COLUMN pgext.universe.vendor          IS 'Extension, kernel, or managed-service vendor.';
COMMENT ON COLUMN pgext.universe.contrib         IS 'Whether the extension is part of PostgreSQL contrib.';
COMMENT ON COLUMN pgext.universe.lang            IS 'Primary implementation language or data format.';
COMMENT ON COLUMN pgext.universe.license         IS 'SPDX expression or project-specific license identifier.';
COMMENT ON COLUMN pgext.universe.tags            IS 'Tags used for search and presentation.';
COMMENT ON COLUMN pgext.universe.version         IS 'Catalog or control-file extension version.';
COMMENT ON COLUMN pgext.universe.url             IS 'Preferred user-facing URL.';
COMMENT ON COLUMN pgext.universe.repo_url        IS 'Source repository or monorepo subdirectory URL.';
COMMENT ON COLUMN pgext.universe.home_url        IS 'Project, product, or extension homepage.';
COMMENT ON COLUMN pgext.universe.doc_url         IS 'Official documentation or specification URL.';
COMMENT ON COLUMN pgext.universe.license_url     IS 'License file or official license page URL.';
COMMENT ON COLUMN pgext.universe.control_url     IS 'Control file or generated definition URL.';
COMMENT ON COLUMN pgext.universe.author_url      IS 'Author, maintainer, or publisher URL.';
COMMENT ON COLUMN pgext.universe.cargo_url       IS 'Cargo.toml or workspace manifest URL.';
COMMENT ON COLUMN pgext.universe.pgxn_url        IS 'PGXN distribution URL.';
COMMENT ON COLUMN pgext.universe.has_bin         IS 'Whether the package includes user-facing executables.';
COMMENT ON COLUMN pgext.universe.has_lib         IS 'Whether the package includes PostgreSQL loadable libraries.';
COMMENT ON COLUMN pgext.universe.need_ddl        IS 'Whether CREATE EXTENSION or equivalent DDL is required.';
COMMENT ON COLUMN pgext.universe.need_load       IS 'Whether LOAD, preloading, or another explicit load step is required.';
COMMENT ON COLUMN pgext.universe.trusted         IS 'Trusted setting from the control file.';
COMMENT ON COLUMN pgext.universe.relocatable     IS 'Relocatable setting from the control file.';
COMMENT ON COLUMN pgext.universe.libs            IS 'Libraries that must be loaded or preloaded.';
COMMENT ON COLUMN pgext.universe.schemas         IS 'Schemas fixed, created, or required by the extension.';
COMMENT ON COLUMN pgext.universe.pg_ver          IS 'Supported PostgreSQL major versions in ascending order.';
COMMENT ON COLUMN pgext.universe.requires        IS 'Forward extension dependencies.';
COMMENT ON COLUMN pgext.universe.required_by     IS 'Reverse dependency cache derived from requires.';
COMMENT ON COLUMN pgext.universe.see_also        IS 'Related, alternative, or complementary extensions.';
COMMENT ON COLUMN pgext.universe.tarball         IS 'Source archive used by Pigsty builds.';
COMMENT ON COLUMN pgext.universe.pgrx_ver        IS 'Effective pgrx build version.';
COMMENT ON COLUMN pgext.universe.rpm_ver         IS 'RPM catalog version.';
COMMENT ON COLUMN pgext.universe.rpm_repo        IS 'RPM package source.';
COMMENT ON COLUMN pgext.universe.rpm_pkg         IS 'RPM package name template.';
COMMENT ON COLUMN pgext.universe.rpm_pg          IS 'PostgreSQL major versions with RPM packages.';
COMMENT ON COLUMN pgext.universe.rpm_deps        IS 'RPM system package dependencies.';
COMMENT ON COLUMN pgext.universe.rpm_build       IS 'Whether Pigsty maintains an RPM build recipe.';
COMMENT ON COLUMN pgext.universe.deb_ver         IS 'DEB catalog version.';
COMMENT ON COLUMN pgext.universe.deb_repo        IS 'DEB package source.';
COMMENT ON COLUMN pgext.universe.deb_pkg         IS 'DEB package name template.';
COMMENT ON COLUMN pgext.universe.deb_pg          IS 'PostgreSQL major versions with DEB packages.';
COMMENT ON COLUMN pgext.universe.deb_deps        IS 'DEB system package dependencies.';
COMMENT ON COLUMN pgext.universe.deb_build       IS 'Whether Pigsty maintains a DEB build recipe.';
COMMENT ON COLUMN pgext.universe.stars           IS 'Latest observed upstream star count.';
COMMENT ON COLUMN pgext.universe.watchers        IS 'Latest observed watcher or subscriber count.';
COMMENT ON COLUMN pgext.universe.forks           IS 'Latest observed fork count.';
COMMENT ON COLUMN pgext.universe.repo_created_at IS 'Upstream repository creation date.';
COMMENT ON COLUMN pgext.universe.last_commit     IS 'Latest known upstream commit date.';
COMMENT ON COLUMN pgext.universe.last_release    IS 'Latest known release, tag, or PGXN publication date.';
COMMENT ON COLUMN pgext.universe.last_active     IS 'Latest credible upstream activity date.';
COMMENT ON COLUMN pgext.universe.checked_at      IS 'Date when upstream metadata was last verified.';
COMMENT ON COLUMN pgext.universe.en_desc         IS 'Short English description.';
COMMENT ON COLUMN pgext.universe.zh_desc         IS 'Short Chinese description.';
COMMENT ON COLUMN pgext.universe.comment         IS 'Maintenance notes, caveats, exceptions, and manual decisions.';
COMMENT ON COLUMN pgext.universe.extra           IS 'Compact attributes not promoted to dedicated columns.';
COMMENT ON COLUMN pgext.universe.mtime           IS 'Date of the latest catalog-level change to the row.';

-- refresh_deps() recalculates the required_by column for all extensions in pgext.universe based on the requires column of other extensions.
CREATE OR REPLACE FUNCTION pgext.refresh_deps()
    RETURNS VOID
    LANGUAGE SQL
AS $function$
WITH reverse_deps AS (
    SELECT dep.name, array_agg(DISTINCT source.name ORDER BY source.name) AS required_by
    FROM pgext.universe AS source
             CROSS JOIN LATERAL unnest(source.requires) AS dep(name)
    WHERE dep.name <> source.name
    GROUP BY dep.name
), calculated AS (
    SELECT target.name, reverse_deps.required_by
    FROM pgext.universe AS target
             LEFT JOIN reverse_deps USING (name)
)
UPDATE pgext.universe AS target
SET required_by = calculated.required_by
FROM calculated
WHERE target.name = calculated.name
  AND target.required_by IS DISTINCT FROM calculated.required_by;
$function$;
...
-----------------------------------
-- Universe to Extension Map
-----------------------------------
-- DROP VIEW IF EXISTS pgext.ext;
CREATE OR REPLACE VIEW pgext.ext AS
SELECT u.id, u.name, u.pkg, u.lead_ext, u.category, 'available'::TEXT AS state,
       u.url, u.license, u.tags, u.version,
       COALESCE(u.extra->>'repo', CASE WHEN u.contrib THEN 'CONTRIB' WHEN u.rpm_repo = u.deb_repo THEN u.rpm_repo WHEN u.rpm_repo IS NULL THEN u.deb_repo WHEN u.deb_repo IS NULL THEN u.rpm_repo ELSE 'MIXED' END) AS repo,
       u.lang, u.contrib, u.name = u.lead_ext AS lead, u.has_bin, u.has_lib, u.need_ddl, u.need_load, u.trusted, u.relocatable,
       u.schemas, u.pg_ver, u.requires, u.required_by AS require_by, u.see_also,
       u.rpm_ver, u.rpm_repo, u.rpm_pkg, u.rpm_pg::TEXT[] AS rpm_pg, u.rpm_deps,
       u.deb_ver, u.deb_repo, u.deb_pkg, u.deb_deps, u.deb_pg::TEXT[] AS deb_pg, u.tarball AS source,
       (u.extra - 'repo') || jsonb_strip_nulls(jsonb_build_object('type', u.kind, 'kernel', u.kernel, 'vendor', u.vendor, 'star', u.stars, 'pgxn', NULLIF(u.pgxn_url IS NOT NULL, false), 'pgrx', u.pgrx_ver, 'rpm', NULLIF(u.rpm_build, false), 'deb', NULLIF(u.deb_build, false), 'lib', array_to_string(u.libs, ','))) AS extra,
       u.en_desc, u.zh_desc, u.comment, u.mtime
FROM pgext.universe AS u
WHERE u.packaged;

COMMENT ON VIEW pgext.ext IS 'Compatibility projection of packaged pgext.universe rows in pgext.extension format.';
