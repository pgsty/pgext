...
const I18N = {
  ...
  'about.p1': ['Every entry is drawn from the <code>pgext</code> catalog database: upstream repositories are crawled, RPM/DEB package indexes are parsed, and GitHub activity is sampled. This prototype snapshot contains <b>{all}</b> entries, including <b>{avail}</b> packaged extensions.',
               '每个条目都来自 <code>pgext</code> 目录数据库：抓取上游仓库、解析 RPM/DEB 包索引并采样 GitHub 活跃度。该原型快照包含 <b>{all}</b> 个条目，其中 <b>{avail}</b> 个为已打包扩展。'],
  ...
  'about.s1': ['pgext.universe — the curated catalog of {all} extensions', 'pgext.universe——{all} 个扩展的策展目录'],
...
function aboutHTML() {
  return '<article class="page wrap">'
    ...
    + '<div><h3>' + (LANG === 'zh' ? '这是什么' : 'What this is') + '</h3><p>' + t('about.p1', { all: fmtInt(N_ALL), avail: fmtInt(N_AVAIL) }) + '</p><p>' + t('about.p2') + '</p></div>'
    + '<div><h3>' + t('about.sources') + '</h3><ul class="roadmap">'
    + '<li><span class="tag">catalog</span>' + t('about.s1', { all: fmtInt(N_ALL) }) + '</li>'
