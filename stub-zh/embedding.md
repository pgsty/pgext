...
上游仓库已归档，并明确声明 `pg_embedding` 已弃用，强烈建议迁移到 `pgvector`。应保持向量维度一致；创建索引时必须指定 `dims`，且它必须等于数组元素数量。

只能使用不含 NULL、坐标均为有限值的稠密数组。C 语言距离计算路径直接读取 `real[]` 原始数据而不检查空元素；余弦距离还会除以两个向量的范数，因此零向量会产生未定义值或 NaN。应使用有代表性的数据验证 HNSW 召回率并调节构建与查询参数。它只适合在迁移规划期间维持现有部署，不应作为新系统的选择。
