...
这是厂商托管的 Ganos 软件，不是可移植的公开扩展。目录记录 7.0，但厂商矩阵会随 PostgreSQL 引擎改变版本与可用性；RDS 小版本策略还可能禁止新建扩展，而现有安装仍可继续使用。厂商基本概念文档指出，GanosBase 与 PostGIS 不能安装在同一模式中，因此启用前必须规划模式。应在可丢弃实例验证 `ST_MakePoint`、patch 统计、几何转换、空间索引、备份恢复和导出可移植性，并为目标环境不存在该组件的迁移场景做好设计。
