...
- [官方上游文档](https://www.alibabacloud.com/help/en/rds/apsaradb-rds-for-postgresql/basic-concepts)
...
`ganos_trajectory` — 对移动对象在时空中的轨迹进行建模、查询与分析。

已复核目录快照记录的版本为 `7.0`、类型为 `standard`、实现语言为 `C`。
应先安装并验证声明的扩展依赖：`ganos_geometry`, `ganos_spatialref`。
