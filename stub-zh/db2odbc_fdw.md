...
不要在 SQL 中保留示例密码，应使用真正的秘密流程，限制用户映射与 ODBC 配置访问，也绝不能把 FDW 或服务器授予 `PUBLIC`。已复核 C 源码在建立连接时通过调试宏记录 DSN、用户名、密码和远端查询。必须把服务器日志视为含凭据数据，在使用前关闭或修补这项日志，并轮换测试中暴露的所有凭据。`sql_query` 定义远端 SQL，必须视为可信管理员输入。
