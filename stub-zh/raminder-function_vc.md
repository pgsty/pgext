...
- [Official database.dev 包页面](https://database.dev/raminder/function_vc)

`raminder-function_vc` — 在 PostgreSQL 函数版本控制中使用此扩展。在管理或自动化上述数据库行为时，请使用链接的上游固定版本作为 API 边界，并在目标 PostgreSQL 构建上进行测试。
...
- `public.rollback_function(func_name text, schema_n text default 'public')` 是一个扩展函数，返回 `text`。
