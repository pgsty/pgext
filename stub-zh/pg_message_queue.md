...
- `pg_mq_create_queue` 与 `pg_mq_drop_queue` 是没有固定 `SET search_path` 的安全定义者函数。虽然安装过程撤销了它们默认的公共执行权，但在修复并审查这一边界之前，不要将其授予不受信角色。
- PostgreSQL 通知是唤醒信号，而不是受保护的载荷投递。数据库用户通常都能监听频道；应保护底层表，不要在通知载荷中放入秘密。
