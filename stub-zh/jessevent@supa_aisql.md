...
- [Official database.dev 包页面](https://database.dev/jessevent/supa_aisql)

`jessevent@supa_aisql` — Supabase 的 AI 推断 SQL 函数。通过 ai.deploy() 自动部署其边缘函数。使用上游链接的固定版本作为 API 边界，并在目标 PostgreSQL 构建上进行测试。
...
- `public.ai_complete(input_text text, system_text text default null, model_name text default null, provider text default 'openai')` 是一个扩展函数并返回 `text`。
