...
PostgreSQL 扩展（522 ext / 439 pkg）归属 16 个分类。
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 1000 | {{< alias "timescaledb" >}} | 2.27.2 | 时序数据库扩展插件 |
...
| 1110 | {{< alias "pg_background" >}} | 2.0 | 在后台运行 SQL 查询 |
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 1500 | {{< alias "postgis" >}} | 3.6.3 | PostGIS 几何和地理空间扩展 |
| 1501 | {{< alias "postgis_topology" "postgis" >}} | 3.6.3 | PostGIS 拓扑空间类型和函数 |
| 1502 | {{< alias "postgis_raster" "postgis" >}} | 3.6.3 | PostGIS 光栅类型和函数 |
| 1503 | {{< alias "postgis_sfcgal" "postgis" >}} | 3.6.3 | PostGIS SFCGAL 函数 |
| 1504 | {{< alias "postgis_tiger_geocoder" "postgis" >}} | 3.6.3 | PostGIS tiger 地理编码器和反向地理编码器 |
| 1505 | {{< alias "address_standardizer" "postgis" >}} | 3.6.3 | 地址标准化函数。 |
| 1506 | {{< alias "address_standardizer_data_us" "postgis" >}} | 3.6.3 | 地址标准化函数：美国数据集示例 |
...
| 1580 | {{< alias "pg_eviltransform" >}} | 0.0.2 | 基于PostGIS ST_Transform 的 BD09/GCJ02 坐标转换扩展 |
...
| 1602 | {{< alias "pgh_hgm" "pghydro" >}} | 2.2.6 | PgHydro 水文地貌分析扩展 |
| 1603 | {{< alias "pgh_output" "pghydro" >}} | 6.6 | PgHydro 输出与报表扩展 |
...
| 1680 | {{< alias "tzf" "pg_tzf" >}} | 0.2.4 | 快速根据GPS经纬度坐标查找时区 |
...
| 1830 | {{< alias "vectorize" "pg_vectorize" >}} | 0.26.1 | 在PostgreSQL中封装RAG向量检索服务 |
...
| 1900 | {{< alias "pgmnemo" >}} | 0.8.3 | 面向 LLM 代理的 PostgreSQL 溯源向量记忆扩展 |
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 2100 | {{< alias "pg_search" >}} | 0.24.0 | ParadeDB BM25算法全文检索插件，ES全文检索 |
...
| 2190 | {{< alias "pg_pinyin" >}} | 0.0.4 | PostgreSQL 拼音转写与检索辅助扩展 |
| 2200 | {{< alias "pg_kazsearch" >}} | 0.1.0 | PostgreSQL 哈萨克语全文检索扩展 |
...
| 2460 | {{< alias "pg_clickhouse" >}} | 0.3.1 | 从PostgreSQL中查询ClickHouse的接口 |
...
| 2510 | {{< alias "pg_partman" >}} | 5.4.3 | 用于按时间或 ID 管理分区表的扩展 |
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 2600 | {{< alias "age" >}} | 1.7.0 | Apache AGE，图数据库扩展 （Deb可用） |
| 2610 | {{< alias "pg_liquid" >}} | 0.1.7 | 受 Liquid 启发的 Datalog 图查询扩展 |
| 2620 | {{< alias "onesparse" "one_sparse" >}} | 1.0.0 | PostgreSQL 18 的稀疏线性代数与图算法扩展 |
| 2630 | {{< alias "graph" "pggraph" >}} | 0.1.7 | PostgreSQL 图查询与遍历扩展 |
| 2640 | {{< alias "pgrdf" >}} | 0.5.0 | PostgreSQL 内 RDF、SPARQL、SHACL 与 OWL 推理扩展 |
| 2650 | {{< alias "pgq" >}} | 3.5.1 | 通用队列的PG实现 |
| 2660 | {{< alias "pgmq" >}} | 1.11.1 | 基于Postgres实现类似AWS SQS/RSMQ的消息队列 |
...
| 2740 | {{< alias "pg_graphql" >}} | 1.5.12 | PG内的GraphQL支持 |
...
| 2840 | {{< alias "pg_ivm" >}} | 1.14 | 增量维护的物化视图 |
| 2850 | {{< alias "pg_incremental" >}} | 1.5.0 | 增量处理流式事件 |
| 2860 | {{< alias "pg_trickle" >}} | 0.81.0 | 为 PostgreSQL 18 提供流式表与差分视图维护 |
| 2870 | {{< alias "pg_durable" >}} | 0.2.2 | 在 PostgreSQL 中使用 SQL 定义可持久化、可恢复的长时间运行函数 |
| 2900 | {{< alias "provsql" >}} | 1.9.0 | PostgreSQL 半环溯源与不确定性管理扩展 |
| 2910 | {{< alias "orioledb" >}} | 1.7 | OrioleDB，下一代事务处理引擎 |
...
| 3060 | {{< alias "plpgsql_check" >}} | 2.9.1 | 对 plpgsql 函数进行扩展检查 |
...
| 3630 | {{< alias "roaringbitmap" "pg_roaringbitmap" >}} | 1.1.0 | 支持RoaringBitmap数据类型 |
...
| 3910 | {{< alias "pgbson" >}} | 2.0.2 | 为 PostgreSQL 提供 BSON 数据类型、比较与访问函数 |
...
| 4080 | {{< alias "pg_net" >}} | 0.20.3 | 用 SQL 进行异步非阻塞HTTP/HTTPS 请求的扩展 (supabase) |
...
| 5020 | {{< alias "pg_rewrite" >}} | 2.1.0 | 在线重写整表，不阻塞读写 |
| 5030 | {{< alias "pg_query_rewrite" >}} | 0.0.5 | 使用 ProcessUtility hook 重写 SQL 语句 |
| 5040 | {{< alias "pg_squeeze" >}} | 1.9.2 | 从关系中删除未使用空间 |
...
| 5090 | {{< alias "pglinter" >}} | 1.1.2 | PG数据库规则检查插件 |
...
| 6410 | {{< alias "pgsentinel" >}} | 1.4.1 | 活跃会话历史 |
| 6420 | {{< alias "system_stats" >}} | 4.0 | PostgreSQL 的系统统计函数 |
...
| 6530 | {{< alias "toastinfo" >}} | 1.6 | 显示TOAST字段的详细信息 |
...
| 7060 | {{< alias "pg_session_jwt" >}} | 0.4.0 | 使用JWT进行会话认证 |
| 7070 | {{< alias "anon" "pg_anon" >}} | 3.0.13 | 数据匿名化处理工具 |
...
| 7120 | {{< alias "pgauditlogtofile" >}} | 1.8.4 | pgAudit 子扩展，将审计日志写入单独的文件中 |
...
| 7310 | {{< alias "credcheck" >}} | 4.7 | 明文凭证检查器 |
...
| 7405 | {{< alias "block_copy_command" >}} | 0.1.5 | 通过可配置的 ProcessUtility hook 阻止 COPY 命令 |
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 8500 | {{< alias "wrappers" >}} | 0.6.0 | Supabase提供的外部数据源包装器捆绑包 |
...
| 8680 | {{< alias "nominatim_fdw" >}} | 1.3 | Nominatim 地理编码接口的 FDW 扩展 |
...
| 8760 | {{< alias "rdf_fdw" >}} | 2.5.0 | 通过 SPARQL 端点访问 RDF 三元组存储的 FDW |
...
| ID | 扩展/包 | 版本 | 描述 |
|:---:|:---|:---|:---|
| 9000 | {{< alias "documentdb" >}} | 0.110 | 微软DocumentDB的API层 |
| 9010 | {{< alias "documentdb_core" "documentdb" >}} | 0.110 | 微软DocumentDB的核心API层实现 |
| 9020 | {{< alias "documentdb_distributed" "documentdb" >}} | 0.110 | DocumentDB多节点模式的API层 |
| 9030 | {{< alias "documentdb_extended_rum" "documentdb" >}} | 0.110 | DocumentDB扩展RUM索引访问方法 |
...
| 9570 | {{< alias "spock" >}} | 5.0.6 | PostgreSQL 多主逻辑复制扩展 |
| 9580 | {{< alias "lolor" >}} | 1.2.2 | 让 PostgreSQL 大对象兼容逻辑复制的扩展 |
| 9590 | {{< alias "pgclone" >}} | 4.3.2 | 在不同环境间克隆 PostgreSQL 数据库、模式、表和函数 |
| 9620 | {{< alias "pgmqtt" >}} | 0.1.0 | PostgreSQL 的 CDC 到 MQTT 代理扩展 |
| 9630 | {{< alias "wal2json" >}} | 2.6 | 用逻辑解码捕获 JSON 格式的 CDC 变更 |
| 9640 | {{< alias "wal2mongo" >}} | 1.0.7 | 使用逻辑解码捕获MongoDB JSON格式的CDC变更 |
| 9650 | {{< alias "decoderbufs" >}} | 3.5.0 | 将WAL逻辑解码为ProtocolBuffer协议的消息 |
