...
3. 扩展管理器会根据操作系统自动适配不同的包格式：
  ...
  - MacOS 目前尚未支持，但一些检索命令仍然可以使用
4. 某些扩展可能有依赖项，安装时会自动解决。
5. `-y` 参数会自动确认所有提示，跳过交互式确认
...
```bash
pig ext link 18                        # 链接 PostgreSQL 18 到 /usr/pgsql
pig ext link pg17                      # 链接 PostgreSQL 17 到 /usr/pgsql
pig ext link /usr/pgsql-16             # 从指定路径链接到 /usr/pgsql
pig ext link null                      # 取消当前 PostgreSQL 链接
```

实现的方式是创建一个 `/etc/profile.d/pgsql.sh` 文件，将 PostgreSQL 目录放置于 PATH 首位：

```bash {filename="/etc/profile.d/pgsql.sh"}
# generated by pig
export PATH="/usr/lib/postgresql/18/bin:$PATH"
export PGHOME="/usr/lib/postgresql/18"
```

例如，您可以通过以下命令组合，安装 PostgreSQL 17，并将其设置为系统默认链接使用的 PG 版本：
