...
详细入门指南请参阅：https://pigsty.io/zh/docs/setup/install/
...
pig sty init -m                # 从 pigsty.cc 镜像源安装
...
- `-m|--mirror`：优先使用 `pigsty.cc` 镜像作为下载源
...
pig sty boot -m                  # 等同于 --region china
...
pig sty conf -m -s                 # 镜像模式，等同于 --region china
...
```bash
pig sty list                     # 列出可用版本
pig sty list -m                  # 从 pigsty.cc 镜像列出版本
```
...
pig sty get -m                   # 从 pigsty.cc 镜像下载
```
