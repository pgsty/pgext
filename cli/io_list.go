...
IO List Generator - generates list pages for pigsty.io (English only)
...
// IOListGenerator generates list pages for pigsty.io
type IOListGenerator struct {
	Cache     *ExtensionCache
	OutputDir string
}

// NewIOListGenerator creates a new IO list generator
func NewIOListGenerator(cache *ExtensionCache, outputDir string) *IOListGenerator {
	return &IOListGenerator{
		Cache:     cache,
		OutputDir: outputDir,
	}
}
...
func (g *IOListGenerator) GenerateExtensionIndexPage(outputPath string) error {
	...
	content := `---
title: "Extensions"
description: "Detailed information for each PostgreSQL extension"
weight: 600
...
func (g *IOListGenerator) GenerateExtensionList(outputPath string) error {
...
`)
	...
	// Summary text
	b.WriteString(fmt.Sprintf("There are **%d** PostgreSQL extensions available in the Pigsty Extension Catalog, in **%d** packages.\n\n",
		extAll, pkgAll))

	// Stats table - all fields centered
	b.WriteString("| **Type** | **All** | **PGDG** | **PIGSTY** | **CONTRIB** | |")
	...
	for _, cat := range g.Cache.Categories {
		...
		b.WriteString("| **Extension** | **Package** | **Version** | **Attr** | **Repo** | **PG Ver** | **Deps** |\n")
		b.WriteString("|:---------|:---------|:--------:|:--------:|:--------:|:-----------:|:---------|\n")

		for _, ext := range exts {
			// Column 1: Extension name - linked to ext page
			extLink := fmt.Sprintf("[`%s`](/ext/e/%s)", ext.Name, ext.Name)

			// Column 2: Package name - linked to upstream URL
			...
			// Column 4: Attributes (attribute flags)
			...
			// Column 6: PG Ver - pgvers shortcode
			pgStr := CCPGVersShortcode(ext.PgVer)

			// Column 7: Dependencies (requires - linked to ext pages)
			depsStr := "-"
...
func (g *IOListGenerator) GeneratePackageList(outputPath string) error {
	var b strings.Builder

	b.WriteString(`---
title: "Packages"
linkTitle: "Packages"
description: "Extensions listed by package name"
weight: 150
icon: fas fa-box
---
...
`)
	...
	b.WriteString("| **Package** | **Lead Ext** | **Version** | **Type** | **RPM** | **DEB** |\n")
	b.WriteString("|:---------|:-----------|:--------:|:--------:|:-------:|:-------:|\n")
	...
	for _, ext := range packages {
		...
		rpmBadge := ioRepoBadgeOrMiss(ext.RpmRepo)
		debBadge := ioRepoBadgeOrMiss(ext.DebRepo)
...
func (g *IOListGenerator) GenerateLanguageList(outputPath string) error {
	var b strings.Builder

	b.WriteString(`---
title: "Language"
linkTitle: "Language"
description: "Extensions by programming language"
weight: 220
icon: fas fa-code
---
...
`)
	...
	for _, lang := range langs {
		...
		isRust := lang == "Rust"
		if isRust {
			b.WriteString("| **Extension** | **Package** | **Version** | **PGRX** | **Type** | **License** | **Description** |\n")
			b.WriteString("|:---------|:-------|:--------:|:--------:|:--------:|:----------:|:---------|\n")
		} else {
			b.WriteString("| **Extension** | **Package** | **Version** | **Type** | **License** | **Description** |\n")
			b.WriteString("|:---------|:-------|:--------:|:--------:|:----------:|:---------|\n")
		}
...
func (g *IOListGenerator) GenerateLicenseList(outputPath string) error {
	var b strings.Builder
...
`)
	...
	for _, lic := range allLicenses {
		...
		b.WriteString("\n--------\n\n")
		b.WriteString(fmt.Sprintf("## %s {#%s}\n\n", lic, anchor))
		b.WriteString(fmt.Sprintf("Extensions using %s license (%d)\n\n", lic, len(exts)))

		b.WriteString("| **Extension** | **Package** | **Version** | **Type** | **Language** | **Description** |\n")
		b.WriteString("|:---------|:-------|:--------:|:--------:|:--------:|:---------|\n")
...
func (g *IOListGenerator) GenerateRepoList(outputPath string) error {
	var b strings.Builder
...
`)
	...
	b.WriteString(fmt.Sprintf("| **Repo** | %s | %s | %s | %s | **Total** |\n",
		CCRepoBadge("PGDG"), CCRepoBadge("PIGSTY"), CCRepoBadge("MIXED"), CCRepoBadge("CONTRIB")))
	b.WriteString("|:------:|:------:|:------:|:------:|:------:|:------:|\n")
	b.WriteString(fmt.Sprintf("| **Extension Count** | %d | %d | %d | %d | **%d** |\n",
		len(pgdgExts), len(pigstyExts), len(mixedExts), len(contribExts), total))
	b.WriteString("{.ext-table}\n\n")

	// PGDG section
	b.WriteString("\n--------\n\n")
	b.WriteString(fmt.Sprintf("## PGDG Repository {#pgdg}\n\nBinary packages from the PostgreSQL Global Development Group, including %d extensions.\n\n", len(pgdgExts)))
	writeIORepoExtTable(&b, pgdgExts)

	// PIGSTY section
	b.WriteString("\n--------\n\n")
	b.WriteString(fmt.Sprintf("## PIGSTY Repository {#pigsty}\n\nPigsty extension repo provides %d additional extensions on top of PGDG.\n\n", len(pigstyExts)))
	writeIORepoExtTable(&b, pigstyExts)

	// MIXED section
	if len(mixedExts) > 0 {
		b.WriteString("\n--------\n\n")
		b.WriteString(fmt.Sprintf("## Both {#mixed}\n\nSome extensions are available in both PIGSTY and PGDG repositories (%d).\n\n", len(mixedExts)))
		writeIORepoExtTable(&b, mixedExts)
	}
...
func writeIORepoExtTable(b *strings.Builder, exts []*Extension) {
	b.WriteString("| **Extension** | **RPM Repo** | **RPM Pkg** | **RPM Ver** | **DEB Repo** | **DEB Pkg** | **DEB Ver** |\n")
	b.WriteString("|:---------|:----------:|:---------|:----------:|:----------:|:---------|:----------:|\n")
...
// writeIOContribExtTable writes the CONTRIB section table
func writeIOContribExtTable(b *strings.Builder, exts []*Extension) {
	b.WriteString("| **Extension** | **Type** | **Version** | **PG Ver** | **Description** |\n")
	b.WriteString("|:---------|:------:|:--------:|:---------|:---------|\n")
...
// ioPlatformListConfig configures a platform-specific package list page (RPM or DEB)
type ioPlatformListConfig struct {
...
func (g *IOListGenerator) GenerateRPMList(outputPath string) error {
	return g.generatePlatformList(outputPath, &ioPlatformListConfig{
		Name:         "RPM",
		OSDesc:       "EL",
		Title:        "RPM List",
		Desc:         "PostgreSQL extension RPM binary packages available on EL systems",
...
func (g *IOListGenerator) GenerateDEBList(outputPath string) error {
	return g.generatePlatformList(outputPath, &ioPlatformListConfig{
		Name:         "DEB",
		OSDesc:       "Debian/Ubuntu",
		Title:        "DEB List",
		Desc:         "PostgreSQL extension DEB binary packages available on Debian/Ubuntu systems",
...
func (g *IOListGenerator) generatePlatformList(outputPath string, cfg *ioPlatformListConfig) error {
	var b strings.Builder
	...
	b.WriteString(fmt.Sprintf("There are **%d** PostgreSQL extensions available on %s systems, in **%d** packages.\n\n",
		extAll, cfg.OSDesc, pkgAll))
	b.WriteString(fmt.Sprintf("**%d** packages are %s exclusive, **%d** %s packages are missing corresponding %s packages.\n\n",
	...
	for _, cat := range g.Cache.Categories {
		...
		b.WriteString("| **Package** | **Version** | **Repo** | **Package** | **PG Ver** | **Deps** |\n")
		b.WriteString("|:----------|:--------:|:--------:|:---------|:-----------:|:---------|\n")

		for _, ext := range pkgs {
			// Column 1: Package - pkg name linked to ext page
			...
			// Column 5: PG Ver - pgvers shortcode
			pgs := cfg.GetPG(ext)
			if ext.Contrib && len(pgs) == 0 {
				pgs = ext.PgVer
			}
...
// If the file already exists, it is kept as-is (user may have custom content).
// If generating fresh, only the YAML frontmatter is written.
func (g *IOListGenerator) GenerateOSIndexPage(outputPath string) error {
	if _, err := os.Stat(outputPath); err == nil {
		logrus.Infof("Keeping existing %s", outputPath)
		return nil
	}
...
func (g *IOListGenerator) GenerateOverviewPage(outputPath string) error {
	var b strings.Builder
...
`, totalExts, activeOSCount))
	b.WriteString("```bash\n")
	b.WriteString("curl -fsSL https://repo.pigsty.io/pig | bash   # Install pig CLI tool\n")
	b.WriteString("pig repo add pigsty pgdg -u                    # Configure repos on your Linux distro\n")
	b.WriteString("pig install pg18                               # Install PostgreSQL 18 kernel from PGDG\n")
	...
	b.WriteString("{.ext-table}\n\n")
	b.WriteString("> See: [Extension List](/ext/list), [RPM List](/ext/rpm), [DEB List](/ext/deb), [Repository](/ext/repo)\n\n")
	...
	// Compatible Systems
	// ══════════════════════════════════════════════════════════════════
	b.WriteString("## Compatible Systems\n\n")
	b.WriteString("| **System** | **Version** | **x86_64** | **aarch64** | **PG Ver** |\n")
	b.WriteString("|:------:|:------:|:----------:|:-----------:|:--------:|\n")
	...
	b.WriteString("{.ext-table}\n\n")
	b.WriteString("> See: [Operating System](/ext/os), [RPM List](/ext/rpm), [DEB List](/ext/deb)\n\n\n\n")

	// ══════════════════════════════════════════════════════════════════
	// Partners
	// ══════════════════════════════════════════════════════════════════
	b.WriteString("--------\n\n## Partners\n\n")
	b.WriteString("Pigsty provides an unparalleled PostgreSQL extension delivery experience. Several PostgreSQL vendors are using it for delivery.\n\n")
	b.WriteString("{{< cardpane >}}\n")
	b.WriteString("{{< card header=\"[**Pigsty**](https://github.com/pgsty/pigsty)\" >}}\n")
	b.WriteString("Out-of-the-box, open-source, enterprise-grade PostgreSQL RDS distribution\n")
	b.WriteString("{{< /card >}}\n")
	b.WriteString("{{< card header=\"[**Omnigres**](https://docs.omnigres.org/quick_start/)\" >}}\n")
	b.WriteString("PostgreSQL as a Platform, application development inside the database\n")
	b.WriteString("{{< /card >}}\n")
	b.WriteString("{{< card header=\"[**AutoBase**](https://autobase.tech/docs/extensions/install)\" >}}\n")
	b.WriteString("Ansible-based PG cluster automation deployment, open-source DBaaS\n")
	b.WriteString("{{< /card >}}\n")
	b.WriteString("{{< /cardpane >}}\n\n")
	b.WriteString("{{< cardpane >}}\n")
	b.WriteString("{{< card header=\"[**TensorChord**](https://github.com/tensorchord)\" >}}\n")
	b.WriteString("Cloud-native AI infrastructure, developers of several well-known PG extensions\n")
	b.WriteString("{{< /card >}}\n")
	b.WriteString("{{< card header=\"[**WENWU IT**](https://w3.ww-it.cn/)\" >}}\n")
	b.WriteString("Enterprise PostgreSQL service provider\n")
	b.WriteString("{{< /card >}}\n")
	b.WriteString("{{< /cardpane >}}\n\n\n")
	...
	// ══════════════════════════════════════════════════════════════════
	b.WriteString("--------\n\n## About\n\n")
	b.WriteString("This project is developed and maintained by [**PGSTY**](https://github.com/pgsty) / [**VONNG**](https://vonng.com), and is open-sourced under the [**Apache 2.0**](https://github.com/pgsty/pig/?tab=Apache-2.0-1-ov-file#readme) license.\n\n")
	b.WriteString("| **Repository** | **Description** |\n")
	b.WriteString("|:------|:------|\n")
	b.WriteString("| [pgsty](https://github.com/pgsty) | PGSTY organization homepage |\n")
	b.WriteString("| [pgsty/pgext](https://github.com/pgsty/pgext) | This website, extension metadata, and management tools |\n")
	b.WriteString("| [pgsty/pigsty](https://github.com/pgsty/pigsty) | PostgreSQL database distribution |\n")
	b.WriteString("| [pgsty/pig](https://github.com/pgsty/pig) | PostgreSQL package manager |\n")
	b.WriteString("| [pgsty/rpm](https://github.com/pgsty/rpm) | RPM build source code |\n")
...
func (g *IOListGenerator) GenerateAllLists() error {
	...
	// Divider: before dimension pages (Repository, Programming Language, License)
...
func (g *IOListGenerator) GenerateDivider(outputPath string, weight int) error {
	content := fmt.Sprintf(`---
title: ""
weight: %d
...
func ioRepoBadgeOrMiss(repo sql.NullString) string {
	if repo.Valid && repo.String != "" {
