...
// ParallelWorker represents a worker pool for concurrent operations
...
// NewParallelWorker creates a new worker pool
func NewParallelWorker(workers int) *ParallelWorker {
...
// Execute runs a function in the worker pool
func (w *ParallelWorker) Execute(ctx context.Context, fn func() error) error {
	...
	go func() {
		...
		if err := fn(); err != nil {
			logrus.Debugf("Worker task failed: %v", err)
		}
	}()
