...
- [Official database.dev package page](https://database.dev/jessevent/ai_sql)

`jessevent@ai_sql` — AI inference SQL functions for Supabase. Self-deploys its own edge function via ai.deploy(). Use it for the corresponding vector, model, or retrieval workflow. Use the pinned upstream revision linked above as the API boundary and test it on the target PostgreSQL build.
...
- `public.ai_complete2(input_text text, system_text text default null, model_name text default null, provider text default 'openai')` is an extension function and returns `text`.
