...
- [Official database.dev package page](https://database.dev/raminder/function_vc)

`raminder-function_vc` — In database version control for postgres functions. Use it when administering or automating the database behavior described above. Use the pinned upstream revision linked above as the API boundary and test it on the target PostgreSQL build.

### Core Workflow

```sql
CREATE EXTENSION "raminder-function_vc";
```
...
- `public.rollback_function(func_name text, schema_n text default 'public')` is an extension function and returns `text`.
...
- The catalog records version `2.0.0`.
