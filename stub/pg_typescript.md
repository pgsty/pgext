...
The control file marks the language trusted, but extension installation is superuser-only and function owners can request capabilities through user-set `typescript.allow_*` settings. Imports default to allowed, while `_pg.execute` defaults to denied until explicitly requested; however, the superuser `typescript.max_*` ceilings are the real upper boundary, and their upstream defaults are effectively uncapped. Set restrictive maximum import, network, file, environment, process, FFI, system, and database-execution policies before granting language use. Remote modules are fetched when functions are created and cached locally, so pin and audit every dependency. Treat untrusted TypeScript as database code, and test memory, timeout, backend-crash, cold-start, and supply-chain behavior before production use.
