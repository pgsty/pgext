...
`db721_fdw` version `0.0.0` is an educational Rust/pgrx foreign data wrapper for the columnar file format used by CMU 15-721 Project 1. A foreign table supplies a server-local `filename` and logical `table` name; the implementation exposes read-only scan callbacks and experiments with projection and simple comparison-filter pushdown.
