...
`ganos_spatialref` — Provides computing and analysis features for spatial references.

The reviewed catalog snapshot records version `7.0`, kind `standard`, and implementation language `Data`.
...
SELECT extversion
FROM pg_extension
WHERE extname = 'ganos_spatialref';
```
